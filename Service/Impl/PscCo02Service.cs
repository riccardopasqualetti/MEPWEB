...
namespace MepWeb.Service.Impl
{
    public class PscCo02Service : IPscCo02Service
    {
        ...
        public async Task<ResponseBase<PscCo02?>> CreateRecordAsync(PscCo02CreateRequest request)
        {
            ...
            if (affected < 1)
            {
                return ResponseBase<PscCo02?>.Failed(PscCo02Errors.InserimentoFalito, "Inserimento falito");
            }
            return ResponseBase<PscCo02?>.Success();
        }

        public async Task<ResponseBase<PscCo02?>> DeleteRecordAsync(string cRisorsa, string cDitta)
        {
            PscCo02 pscCo02 = await _db.PscCo02s.SingleOrDefaultAsync(e => e.CRisorsa == cRisorsa && e.CDitta == cDitta);
            if (pscCo02 == null)
            {
        ...
        public async Task<ResponseBase<PscCo02?>> UpdateRecordAsync(PscCo02UpdateRequest request)
        {
            ...
            if (result < 1)
            {
                return ResponseBase<PscCo02?>.Failed(PscCo02Errors.AggiornamentoFalito, "Aggiornamento falito");
            }
            return ResponseBase<PscCo02?>.Success();
